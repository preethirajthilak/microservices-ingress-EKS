pipeline {
    ...
    environment {
        DOCKER_HUB_REPO = 'preethi90/clickops'
        K8S_CLUSTER_NAME = 'clickops-cluster'
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git 'https://github.com/preethirajthilak/microservices-ingress-EKS.git'
            }
        }
        ...
        stage('Deploy to Kubernetes') {
            steps {
                ...
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-creds']]) {
                        sh "sed -i 's|preethi90/clickops:latest|preethi90/clickops:${env.IMAGE_TAG}|g' k8s/deployment.yaml"
